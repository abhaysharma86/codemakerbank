...
import com.codemakerbank.filter.*;
...
public class SecurityConfig {
  ...
  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity security) throws Exception{
      ...
      security.sessionManagement(smc->smc.invalidSessionUrl("/invalidSession").maximumSessions(2).maxSessionsPreventsLogin(true))
              .csrf(csrfConfig->csrfConfig.csrfTokenRequestHandler(csrfTokenRequestAttributeHandler).ignoringRequestMatchers("/api/v1/apiLogin","/api/v1/Account/contact/**")
                ...
                .requestMatchers("/api/v1/Account/contact/**","api/v1/apiLogin").permitAll());
    ...
    @Bean
    public AuthenticationManager authenticationManager(CodeMakerBankUserDetailsService userDetailsService,
                                                       PasswordEncoder passwordEncoder) {
        CodeMakerAuthenticationProvider authenticationProvider =
                new CodeMakerAuthenticationProvider(userDetailsService, passwordEncoder);
